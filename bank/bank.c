...
bool transfer(int from_idx, int to_idx, int amount) {
    ...
    int ret_s = pthread_mutex_unlock(&accounts[second].mutex);
    if (ret_s != 0) {
        fprintf(stderr, "[WARN] Unlock on second mutex (idx: %d) failed with code %d: %s\n",
                second, ret_s, strerror(ret_s));
    }
    int ret_f = pthread_mutex_unlock(&accounts[first].mutex);
    if (ret_f != 0) {
        fprintf(stderr, "[WARN] Unlock on first mutex (idx: %d) failed with code %d: %s\n",
                first, ret_f, strerror(ret_f));
    }
